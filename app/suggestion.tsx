...
export function Sugestion(props: SugestionFormProps) {
    ...
    const onSubmit: SubmitHandler<FormValues> = (data) => {
        const finalData = { ...data, garcom: garcomName };

        props.onSubmit?.(finalData);

        addDoc(collection(db, "sugestions"), finalData)
            .then(() => console.log("Sugestão enviada!"))
            ...
            <Stack gap='4'>
                <Controller
                    ...
                    render={({ field }) => (
                        ...
                        >
                            <NumberInput.Label mt='30px' gap='8'>
                                Dê uma nota de 0 a 10 ao Garçom{" "}
                                <Text as='span' fontWeight='extrabold'>{garcomName}</Text>
                            </NumberInput.Label>
                ...
                <Field.Root invalid={!!errors.atendimentoAvaliation} mt='10px' mb='10px'>
                    ...
                    <NativeSelect.Root size='sm' width='240px'>
                        <NativeSelect.Field {...register('atendimentoAvaliation')}>
                ...
                <HStack gap='4'>
                    ...
                    <Button type='submit' disabled={isLoading || garconInvalido}>
                        {isLoading ? 'Aguarde...' : 'Concluir Avaliação'}
                    </Button>
                </HStack>
            </Stack>
        </form>
